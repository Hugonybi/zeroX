...
interface WishlistNotificationContextType {
  showSuccess: (message: string, artworkTitle?: string) => void;
  showError: (error: WishlistError | string, artworkTitle?: string) => void;
...
const WishlistNotificationContext = createContext<WishlistNotificationContextType | undefined>(undefined);

interface WishlistNotificationProviderProps {
  children: ReactNode;
}

export function WishlistNotificationProvider({ children }: WishlistNotificationProviderProps) {
  const [notifications, setNotifications] = useState<WishlistNotification[]>([]);

  const addNotification = useCallback((notification: Omit<WishlistNotification, 'id'>) => {
  ...
  const showError = useCallback((error: WishlistError | string, artworkTitle?: string) => {
  ...
  const value: WishlistNotificationContextType = {
    showSuccess,
    showError,
    showInfo,
    showActionNotification,
...
interface WishlistNotificationToastProps {
  notification: WishlistNotification;
  onClose: () => void;
}

function WishlistNotificationToast({ notification, onClose }: WishlistNotificationToastProps) {
  const handleAction = () => {
...
export function useWishlistNotifications(): WishlistNotificationContextType {
  const context = useContext(WishlistNotificationContext);
  if (context === undefined) {
    throw new Error('useWishlistNotifications must be used within a WishlistNotificationProvider');
  }
  return context;
}

// Predefined notification messages for common wishlist operations
export const WishlistNotificationMessages = {
  // Success messages
  ITEM_ADDED: 'Added to wishlist',
  ITEM_REMOVED: 'Removed from wishlist',
  MOVED_TO_CART: 'Moved to cart',
  WISHLIST_CLEARED: 'Wishlist cleared',
  WISHLIST_MIGRATED: 'Wishlist items migrated to your account',
  
  // Error messages
  ITEM_UNAVAILABLE: 'Item is no longer available',
  NETWORK_ERROR: 'Network error. Please check your connection',
  STORAGE_ERROR: 'Storage error. Please try clearing your wishlist',
  VALIDATION_ERROR: 'Invalid item. Please try again',
  UNKNOWN_ERROR: 'Something went wrong. Please try again',
  
  // Info messages
  AVAILABILITY_CHANGED: 'Item availability has changed',
  ITEM_SOLD: 'Item has been sold to another buyer',
  GUEST_WISHLIST_NOTICE: 'Sign in to save your wishlist across devices',
  PRICE_CHANGED: 'Price has changed for wishlisted item',
} as const;